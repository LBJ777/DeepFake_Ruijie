...
class FreqPRISMConfig:
    ...
    @property
    def source_train_manifest(self) -> Path | None:
        value = self.raw["dataset"].get("source_train_manifest")
        if not value:
            return None
        path = Path(str(value))
    ...
    @property
    def target_test_manifest(self) -> Path | None:
        value = self.raw["dataset"].get("target_test_manifest")
        if not value:
            return None
        path = Path(str(value))
        if not path.is_absolute():
