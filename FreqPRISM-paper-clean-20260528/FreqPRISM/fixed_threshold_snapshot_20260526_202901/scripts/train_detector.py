...
from data.manifests import filter_image_samples_by_manifest
...
def train_artifact_prior(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        f"variant={args.train_variant} batch_size={int(args.batch_size)}",
        flush=True,
    )
...
def train_semantic_prior(args: argparse.Namespace) -> None:
    ...
    source_all = collect_labeled_images(args.source_root)
    source_train = select_per_label(source_all, max_per_label=int(args.train_per_label), skip_per_label=0)
...
def train_residual_prior(args: argparse.Namespace) -> None:
    ...
    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)
    samples = filter_image_samples_by_manifest(collect_labeled_images(args.source_root), args.train_manifest or None)
