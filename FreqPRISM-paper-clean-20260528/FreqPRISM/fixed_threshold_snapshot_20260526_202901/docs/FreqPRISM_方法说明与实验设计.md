...
- **Fixed source-only fusion**：用固定阈值和固定融合权重得到最终分数，不使用目标集标签做训练、调参、阈值选择或 epoch 选择。
...
其中 `alpha_pos / alpha_neg` 会根据图像是否高分辨率以及 tile 增益状态切换。当前固定参数为：
...
这里 `gamma=0.08` 很小，说明 residual prior 是稳定修正项，而不是主导分支。
...
|------|----------|---------|----------|
| GAN / face / translation | `96.00` | `99.76` | `99.76` |
| Diffusion / commercial / text-to-image | `89.77` | `98.68` | `98.61` |
...
| CycleGAN | `98.64` | `99.93` | `97.35` | `99.92` |
| DALLE2 | `98.45` | `99.81` | `99.00` | `97.90` |
| Glide | `97.86` | `99.81` | `99.13` | `96.58` |

这些结果说明 artifact + semantic + residual 的组合并没有只对 ProGAN 过拟合，在多个未见 generator 上仍保持很高排序能力。

### 9.2 当前尾部集合

当前主要薄弱点不是 AP 大幅崩塌，而是固定阈值下部分 diffusion/text-to-image 集合 fake-side recall 不足：

| generator | Acc | AP | r_acc | f_acc | 主要问题 |
|-----------|-----|----|-------|-------|----------|
| Wukong | `77.65` | `96.64` | `99.35` | `55.95` | fake 侧漏检较多 |
| SD v1.4 | `80.68` | `97.15` | `99.30` | `62.07` | fake 侧漏检较多 |
| SD v1.5 | `80.69` | `97.32` | `99.31` | `62.08` | fake 侧漏检较多 |
| Midjourney | `86.22` | `98.62` | `99.30` | `73.13` | fake 侧仍偏保守 |

这组现象很关键：这些集合的 AP 仍然较高，说明排序能力并不差；但固定 `0.50` 阈值偏保守，导致 fake-side recall 低于 real-side recall。

另一个不同类型的尾部是：

| generator | Acc | AP | r_acc | f_acc | 主要问题 |
|-----------|-----|----|-------|-------|----------|
| GauGAN | `87.44` | `99.63` | `74.92` | `99.96` | real 侧 false positive 偏多 |
| BigGAN | `93.50` | `99.55` | `87.55` | `99.45` | real 侧 false positive 偏多 |

这说明不同尾部集合的错误方向并不相同：diffusion 尾部主要是 fake-side 保守，部分 GAN 尾部则是 real-side domain mismatch。
...
### 10.3 Residual prior 作为低权重修正项

NPR residual 对上采样/重采样痕迹敏感。将它以 `gamma=0.08` 融入最终 logit，可以补充 artifact/semantic 分支，但不会让残差分支主导最终判断。

### 10.4 固定融合避免目标集泄漏

很多 detector 的风险不在模型本身，而在 target 上调阈值、挑权重、挑 epoch 或只报告有利 generator。FreqPRISM 明确把这些选择固定在配置中，并在 protocol 中记录：
...
因此，它更适合作为严格复现实验中的 single-detector demo。
