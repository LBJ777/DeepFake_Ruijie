...
def _with_optional_path(argv: list[str], flag: str, path: Path | None) -> list[str]:
    if path is None:
        return argv
    return [*argv, flag, str(path)]
...
def main() -> None:
    ...
    if "artifact" in stages:
        ...
        argv = _with_optional_path(argv, "--train_manifest", config.source_train_manifest)
        _run(
            _with_progress_flag(argv, bool(args.no_progress)),
