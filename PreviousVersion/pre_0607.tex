...
\midrule
Full FUSEGuard & 94.82 & 0.00 & 94.69 & 94.95 \\
...
The ablation results clarify why source-guarded fusion is useful. Artifact evidence provides the main fake-side anchor: removing it lowers mean Acc from 94.82 to 84.06 and fake accuracy from 94.95 to 71.50 on AIGCDetectBenchmark. The other priors play narrower but complementary roles. Semantic evidence mainly improves real-side behavior, residual evidence adds a non-redundant correction, and native-tile evidence recovers local traces that can be weakened by whole-image resizing. This pattern supports constrained fusion, but it also defines a boundary: FUSEGuard should be read as artifact-led evidence integration, not as four independent detectors that each solve the task alone.
